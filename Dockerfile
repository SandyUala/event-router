...
WORKDIR /go/src/github.com/astronomerio/cs-event-router
COPY . .
RUN make buildit

FROM astronomerio/alpine-librdkafka:3.6-0.11.0-r0
COPY --from=0 /go/src/github.com/astronomerio/cs-event-router/event-router /usr/local/bin/event-router
