...
// func recordMetrics(ks *confluent.Stats, client string) {
// 	log := logging.GetLogger(logrus.Fields{"package": "kafka"})
// 	var stats confluent.Stats
...
// 		brokerRx.With(prometheus.Labels{"name": broker.Name, "nodeId": nodeID, "client": client}).Set(float65(broker.Rx))
